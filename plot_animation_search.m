...
if isempty(h_r_est_plot)
    ...
    h_r_est_plot_h=plot((x_vec_all(:,1)+r_est_h(:,1).*cos(theta_data))/plot_scaling,(x_vec_all(:,2)+r_est_h(:,1).*sin(theta_data))/plot_scaling,'--m','Linewidth',2,'EraseMode','normal', 'color', 'blue');
...
if isempty(h_geo_est_plot)
    h_geo_est_plot=plot((centre_geo_circle(:,1)+radius_geo_circle(:,1).*cos(theta_data))/plot_scaling,(centre_geo_circle(:,2)+radius_geo_circle(:,1).*sin(theta_data))/plot_scaling,'-m','Linewidth',2,'EraseMode','normal', 'color', 'red');
